...
#define PPLog(...)
#endif
@interface HourNumerButton()<UITextFieldDelegate>
/** 减按钮*/
...
@end
@implementation HourNumerButton
#pragma mark - 初始化
...
{
    return [[HourNumerButton alloc] initWithFrame:frame];
}
#pragma mark - 设置UI子控件
- (void)setupUI
{
    ...
    _textField.text = [NSString stringWithFormat:@"%ld分钟",_minValue];
    _textField.textColor=[tools colorWithHex:0x333333];
...
- (void)layoutSubviews
{
    ...
    if (_decreaseHide)
    {
        ...
        _textField.text = [NSString stringWithFormat:@"%ld分钟",_minValue-1];
...
{
    NSString *minValueString = [NSString stringWithFormat:@"%ld分钟",_minValue];
    NSString *maxValueString = [NSString stringWithFormat:@"%ld分钟" ,_maxValue];
...
- (void)increase
{
    ...
    if (number <= _maxValue)
    {
        ...
        _textField.text = [NSString stringWithFormat:@"%ld分钟", number];
        _numberBlock ? _numberBlock(_textField.text) : nil;
        _delegate ? [_delegate pp_numberButton:self number:_textField.text] : nil;
    }
    else
...
{
    _minValue = minValue;
    _textField.text = [NSString stringWithFormat:@"%ld分钟",minValue];
}
