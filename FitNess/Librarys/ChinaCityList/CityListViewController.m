...
//    CompetitionViewController *ComVC;
...
- (void)viewDidLoad
{
    ...
    [self setNavigationWithTitle:@"选择地域"];
    [self getCityData];
    if (_cityName)
    {
        [self showFinaTownData:_cityName];
    }
    //3自定义背景
    ...
    UIImageView *searchBg = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"bg.png"]];
   ...
   _backView=[[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(_searchText.frame)+65, self.view.frame.size.width, 40)];
...
//当前全城
    _cityLabel=[[UILabel alloc]initWithFrame:CGRectMake(20, 5,250, 30)];
//    _cityLabel.text=[NSString stringWithFormat:@"当前:%@全城",_cityName];
    _cityLabel.text=[NSString stringWithFormat:@"当前: 成都市"];
    _cityLabel.textColor=[UIColor darkGrayColor];
    [_backView addSubview:_cityLabel];
    ...
    _btn.frame=CGRectMake(self.view.frame.size.width-100, 5,80,30);
    _btn.titleLabel.font=[UIFont systemFontOfSize:15];
    [_btn setTitle:@"选择区县" forState:UIControlStateNormal];
...
//    [_btn setTitleEdgeInsets:UIEdgeInsetsMake(0, -40, 0, _btn.frame.size.width)];
    [_btn setImageEdgeInsets:UIEdgeInsetsMake(0, 0, 0, -120)];
    _isOpen=NO;
    [_btn addTarget:self action:@selector(areaClick:) forControlEvents:UIControlEventTouchUpInside];
    [_backView addSubview:_btn];
    
    //tableView
    _tableView = [[UITableView alloc] initWithFrame:self.view.bounds style:UITableViewStylePlain];
    _tableView.frame           = CGRectMake(0,searchView.frame.origin.y+searchView.frame.size.height+40, self.view.frame.size.width, self.view.frame.size.height-64);
...
- (void)textChange:(UITextField*)textField
{
    ...
    _tableView.frame=CGRectMake(0, 64+44, self.view.frame.size.width, self.view.frame.size.height-64-40);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(nonnull NSIndexPath *)indexPath
{
    ...
    if (_delegate) {
        ...
        [self showFinaTownData:cityName];
        [self.view addSubview:_backView];
        _cityLabel.text=[NSString stringWithFormat:@"当前:%@全城",cityName];
    }
...
-(void)loadAndDealWithData{
    ...
    NSDictionary *dicAllValue=[NSDictionary dictionaryWithContentsOfFile:loadPath]; // 得到Plist文件中所有的数据
    // 下面是处理数据
